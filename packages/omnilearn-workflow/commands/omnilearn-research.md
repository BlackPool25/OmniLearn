...
| **Confidence calibration** | Every finding has an evidence-bound confidence: ≥85% needs L1–2 sources; 70–85% needs L3 + survived falsification; below is contested, never a number. E.g. "Hybrid search reaches 90–100% recall (70-85%, L2, survived falsification)" not "some evidence suggests." |
...
| `google_search` / `websearch_web_search_exa` | Every phase | Primary research tool — minimum 8-15 queries per phase |
...
4. MUST DO — Scientific Literature Review Methodology (your angle):
   ...
   - STEP 1 — Search Strategy: construct 8+ distinct queries with Boolean operators (AND/OR/NOT, quotes, site:) focused on academic sources; minimum 8 searches.
   - STEP 1b — Contradiction-seeking queries (MANDATORY, ≥2 dedicated queries per angle): after your neutral coverage queries, run at least 2 queries whose explicit goal is to find disagreement, refutation, or negative results. Falsification is mandatory, not optional. Log each contradiction-seeking query verbatim in the search-strategy table with its own N retrieved → N screened → N kept counts.
   - STEP 1c — Primary-source grounding via arxiv (MANDATORY for academic reviewer): pick the top 3–5 papers most central to your angle (title-match pre-check: confirm the abs-page title matches the PDF header before trusting the file). Download + extract with section-cited notes via pymupdf (`pip install arxiv-dl pymupdf`); record one note line per paper: `<ID> — <finding/number> (Sec <X> p.<Y>)`. If any ID 404s or the PDF is not text-extractable, record `UNVERIFIED (<ID>: <reason>)` + fallback (abs page) instead of inventing content.
...
4. MUST DO — Scientific Literature Review Methodology (your angle):
   - **CURRENT DATE: {CURRENT_DATE}**
   - STEP 1 — Search Strategy: construct 8+ distinct queries with Boolean operators (AND/OR/NOT, quotes, site:) focused on industry/practical sources; minimum 8 searches.
   - STEP 1b — Contradiction-seeking queries (MANDATORY, ≥2 dedicated queries per angle): after your neutral coverage queries, run at least 2 queries whose explicit goal is to find disagreement, refutation, or negative results. Falsification is mandatory, not optional. Log each contradiction-seeking query verbatim in the search-strategy table with its own N retrieved → N screened → N kept counts.
   - STEP 1c — Primary-source grounding via arxiv (if topic has arXiv-relevant tech content): pick the top 3–5 papers most central to your angle (title-match pre-check: confirm the abs-page title matches the PDF header before trusting the file). Download + extract with section-cited notes via pymupdf (`pip install arxiv-dl pymupdf`); record one note line per paper: `<ID> — <finding/number> (Sec <X> p.<Y>)`. If any ID 404s or the PDF is not text-extractable, record `UNVERIFIED (<ID>: <reason>)` + fallback (abs page) instead of inventing content. Do not run arxiv-dl on DOI-only journal articles — use publisher-PDF fallback.
...
4. MUST DO:
   ...
   - STEP 0 — Search-strategy table (PRISMA-S Item 8, per angle; reporting artifact owned by the merger; per-angle rows filled from reviewer reports):

   ### Search-strategy table (PRISMA-S Item 8, per angle)

   | angle | database/connector | full query string (verbatim, operators + limits) | limits | N retrieved → N screened → N kept |
   |-------|--------------------|--------------------------------------------------|--------|------------------------------------|
   | academic | <e.g. google_search> | `<paste full query string>` | <e.g. date range, filetype, site:> | <N> → <N> → <N> |
   | academic | <e.g. websearch_web_search_exa> | `<paste full query string>` | <limits> | <N> → <N> → <N> |
   | industry | <e.g. google_search> | `<paste full query string>` | <limits> | <N> → <N> → <N> |
   | industry | <e.g. context7_query-docs> | `<paste library ID + query>` | <version/scope> | <N> → <N> → <N> |

   Rule: record ≥1 full query string per angle verbatim (operators + limits) so a third party can rerun it (PRISMA-S Item 8 p.2: "such that it could be repeated").

   ### Inclusion / exclusion criteria

   Include:
   - <criterion, e.g. directly addresses the research question angle>
   - <criterion, e.g. primary source or Level 1–3 evidence for academic angle>

   Exclude (with reason logged per exclusion):
   - <criterion + reason, e.g. opinion without data — reason: below evidence bar>
   - <criterion + reason, e.g. duplicate of an already-kept source — reason: dedup>

   ### Flow-count line (PRISMA-S Item 17 / Fig.1 p.14)

   `Flow: <N> retrieved → <N> screened → <N> eligible → <N> included (<R> excluded at screening: <reasons>; <R> excluded at eligibility: <reasons>).`
   ...
   - STEP 2 — Contradiction mapping (THE key output — gaps for hypothesis generation come from contradictions, not absences): explicitly identify where sources disagree across angles. For each contradiction: what do they disagree about? How strong is each side? Emit the claim-vs-claim merger map into contradictions-map.md:

   | # | Claim A (+ sources) | Claim B (+ sources) | Root of disagreement | Resolution status | Current balance |
   |---|---------------------|---------------------|----------------------|-------------------|-----------------|
   | 1 | <claim + [S1, S2]> | <counter-claim + [S3]> | <e.g. different datasets / time windows / definitions / methods> | Unresolved / Leaning-A / Leaning-B / Resolved | <which side the weight of evidence currently favors and why, 1 line> |

   Rule: every map row needs ≥1 source per side and a non-empty Root of disagreement. Resolution status is one of the four values above; Unresolved is a correct outcome.
...
4. MUST DO — Popperian Hypothesis Generation:
   ...
   - **Verification-method**: {how this sub-hypothesis will be tested: which retrieval/evidence type decides it}
   - **Expected-outcome**: {what MUST be observed if the sub-hypothesis holds}
...
5. MUST NOT DO:
   ...
   - Do NOT forget to specify alternative explanations — a hypothesis without alternatives is incomplete
    ...
    - Do NOT run any retrieval before each sub-hypothesis has a registry entry with verification-method + expected-outcome + falsification criterion (hypothesis-first ordering per HDRI Sec 3.3 p.6).
...
4. MUST DO:
   ...
   - RELEVANCE GATE (POPPER Table 3 p.6, load-bearing for Type-I control): before running each query, ask "does this query directly target H?" — if no, rewrite or drop the query. Log dropped/rewritten queries in evidence-log.md.
   - CASCADE ORDER (CiteTracer Sec 4 p.4; HDRI gap-pass +14% Abstract p.1): retrieve in this order — cache → direct URL/DOI → parallel scholar connectors → web search. Record a per-source verdict (hit/miss/unverifiable) in evidence-log.md. After the first pass, run one explicit gap-pass: list what is still missing per sub-hypothesis, then issue targeted queries for exactly those gaps.
...
- FIELD-LEVEL CITATION CHECK (CiteTracer Sec 4.3 p.6, rule not LLM opinion): for each cited source verify title/authors/venue/year/DOI separately and label each source R (real), P (potential), or H (hallucinated). Template per source: `Title: {match/mismatch} | Authors: {match/mismatch} | Venue: {match/mismatch} | Year: {match/mismatch} | DOI: {match/mismatch} | Label: {R/P/H}`. Disposition: full match → R, keep; partial match → P, flag for re-verification via cascade before use; no match → H, remove with note 'Removed — source could not be verified.'

### Step 3.5: Claim lock (inside Phase 3)

Step 3.5 inside Phase 3 — not a phase, no new team/files. This is a gate within Phase 3 evidence gathering, not a phase, not a team, not a deliverable file.

- HIGH-RISK RULE (POPPER Sec 2.3 p.3 cumulative-evidence style; POPPER failure audit p.15): a high-risk non-code claim locks only with ≥2 independent domains + counter-search + primary source. Code-execution verification is out of scope — non-code claim lock only.
- ANNEX TEMPLATE (Unresolved/Refuted are correct outcomes, not failures):

  # Claim-lock annex

  ## Locked: {claim} — {≥2 independent domains + counter-search + primary source listed}
  ## Unresolved: {claim} — {what is missing to lock}
  ## Refuted: {claim} — {falsification criterion met by which source}

- SELF-ENFORCING RULE: unlocked claims may not appear in synthesis.
...
4. MUST DO — Scientific Synthesis:
   ...
   STEP 3a — Assign confidence with the evidence-bound rubric (mechanical, no vibes):

   | Band | Numeric range | Requirement |
   |------|---------------|-------------|
   | High | ≥85% | L1–2 sources backing the claim |
   | Moderate | 70–85% | L3 sources AND the claim survived falsification |
   | Contested | below 70% (no numeric range claimed) | label `contested`, never a number |

   Rules: a range narrower than its band is fine (e.g. 75–85% inside Moderate); a range
   straddling bands is forbidden — drop to the lower band. Anything below 70% is
   `contested`, not a low number. Confidence without a cited evidence level fails the gate.

   Worked good-vs-vague pairs (follow the GOOD shape, never the VAGUE shape):

   1. VAGUE: `Some evidence suggests hybrid search improves recall.`
      GOOD: `Hybrid search (DB + 1 snowballing iteration) reaches 90–100% recall (70-85%, L2, survived falsification: S03/S04/S05).`
   2. VAGUE: `Scopus seems better for peer-reviewed-only SLRs.`
      GOOD: `Scopus beats GS on operability for peer-reviewed-only SLRs (70-85%, L2, survived falsification: S11 head-to-head + S12 tool evidence; single-case limit noted).`
   ...
   ## Finding 1: {claim}
   - **Confidence**: [band + evidence per STEP 3a rubric, or `contested`]
   - **Evidence strength**: [GRADE level + study count]
   - **Key sources**: [links]
   - **Remaining uncertainty**: [what we still don't know]
   - **Alternative explanations not ruled out**: [≥1 REQUIRED — at least one rival reading of the same evidence]
   - **Would be overturned by**: [REQUIRED — one concrete future observation with a numeric tripwire, e.g. "a pre-registered live SE SLR where DB+1-iteration recall is <85%"]
   ...
   # Recommendations
    ...
    Based on the findings, what should the user DO with this information? Write one block per recommendation:

    ## Recommendation {N}: {decision}
    - **Decision**: [explicit choose-X-over-Y / do-X-when-C — no direction-without-decision]
    - **Tradeoffs**: [≥1 concrete cost, risk, or boundary condition with a number or source where known]
    - **Next actions**: [≥1 verb + artifact + owner (+ deadline where known)]
    - **Drop condition**: [when to abandon the decision — numeric tripwire]

    Reference the scoring rubric (`scoring-rubric.md` levels 4–5): level 4 = decision + tradeoff + next action with one element weak; level 5 = all three concrete and verifiable without asking the author. Platitudes (`more research needed`) fail.
...
| arxiv step done: academic angle fetched top 3–5 papers via arxiv-dl with pymupdf section-cited notes | 1 | Verifier: academic-angle reviewer output — re-run arxiv step until notes cite sections |
| Claim-lock enforced: zero unlocked high-risk non-code claims appear in synthesis | 3, 4 | Verifier: cross-check findings.md claims against claim-lock registry — move unlocked claims to Unresolved/Refuted annex |
| Contradiction floor met via force-seek: ≥2 contradictions after ≥2 dedicated contradiction-seeking queries per angle | 1 | Verifier: contradictions-map.md row count + query log — force-seek with broader scope until floor met |
| Falsification coverage: every hypothesis has ≥1 recorded falsification attempt in evidence-log | 3 | Verifier: evidence-log per-hypothesis grep — run falsification agent for uncovered hypotheses |
| Field-level verification labels present on every citation (title/authors/venue/year/DOI + R/P/H label) | 3 | Verifier: source-table.md column check — re-verify unlabeled citations before synthesis |
| Confidence-rubric compliance: every finding carries band + evidence level or `contested`, no straddling ranges | 4 | Verifier: findings.md template-field grep — rewrite non-compliant confidence lines per STEP 3a rubric |
...
| Claim lock fails (high-risk claim cannot meet ≥2 independent domains + counter-search + primary source) | Leave the claim Unresolved or Refuted in the annex — unlocked claims may not appear in synthesis. Do NOT force it into synthesis. |
| arxiv fetch fails (arxiv-dl error, no PDF, or pymupdf unreadable) | Fall back to abstract + publisher page via DOI; log `arxiv-fallback: {paper-id} — {reason}` in evidence-log.md and flag the source R/P/H accordingly |
| Claim lock fails (high-risk claim cannot reach ≥2 independent domains + counter-search + primary source) | Move claim to Unresolved/Refuted annex with the missing leg named; log `claim-unlocked: {claim} — missing {leg}` in evidence-log.md; synthesis must not cite it |
| Zero contradictions found after force-seek | Broaden scope per force-seek method and re-run; if still zero, log `no-contradictions-after-forceseek: {angle}` in contradictions-map.md and mark the angle settled-or-shallow in synthesis |
